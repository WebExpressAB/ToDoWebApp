...
function uppdateraOutput(day) {
    const list = toDoLists[day].map((item, index) => `
        <li draggable="true" class="${item.checked ? "checked" : ""}"
            onclick="toggleTaskChecked('${day}', ${index})"
...
function toggleTaskChecked(day, index) {
    // Toggla checked-status
    toDoLists[day][index].checked = !toDoLists[day][index].checked;
